...
export default function PlayerHeadshot({ espnId, sleeperId, name, size = "sm", shape = "circle" }) {
  const cls = SIZES[size] ?? SIZES.sm;
  const shapeCls = shape === "square" ? "rounded-xl" : "rounded-full";
  const sources = [];
  ...
  if (sources.length === 0 || srcIndex >= sources.length) {
    return (
      <div className={`${cls} ${shapeCls} bg-gray-200 text-gray-500 flex items-center justify-center font-bold flex-shrink-0`}>
        {initials(name)}
      </div>
    );
  }
  ...
  return (
    <img
      ...
      alt={name || ""}
      className={`${cls} ${shapeCls} object-cover flex-shrink-0 bg-gray-100`}
