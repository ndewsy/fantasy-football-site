  ...
  useEffect(() => {
    async function load() {
      ...
      const [subResult, postsResult, profileResult, rankingsResult, ownProfileResult, activeCreatorsResult] = await Promise.all([
        ...
        supabase.from("profiles").select("display_name, handle, bio, announcement, logo_url").eq("creator_id", "ffhuddle").eq("is_creator", true).maybeSingle(),
    ...
    <main className="min-h-screen text-ink lg:pl-56">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="flex flex-wrap items-center gap-4 mb-12">
          <CreatorAvatar logoUrl={creatorProfile?.logo_url} initials="FFH" colorClass="bg-blue-600" size="lg" />
          <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {posts.map((post) => (
              <PostCard
                ...
                creatorName={creatorProfile?.display_name || "FantasyFootballHuddle"}
