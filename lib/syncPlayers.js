...
export async function syncPlayers() {
  ...
  // Sleeper labels team defenses "DEF" — stored as "DST" to match this site's
  // own terminology. Their entries have no full_name/espn_id, both handled
  // by the existing fallbacks below (name from first+last, espn_id -> null).
  const active = Object.values(raw).filter(
    p => p.active && (POSITIONS.has(p.position) || p.position === 'DEF') && p.team
  );
  ...
  for (const p of active) {
    ...
    const key = `${normalize(name)}|${position}`;

    const height_inches = p.height ? parseInt(p.height, 10) || null : null;
    const weight_lbs = p.weight ? parseInt(p.weight, 10) || null : null;
    const age = typeof p.age === 'number' ? p.age : null;
    // Drives the Start/Sit rookie-debut fallback (lib/fantasyProjection.js) —
    // a direct fact from Sleeper, not derived from years_exp, so it stays
    // correct regardless of practice-squad/injury-exemption years.
    const rookie_year = p.metadata?.rookie_year ? parseInt(p.metadata.rookie_year, 10) || null : null;
