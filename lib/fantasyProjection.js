...
// game), and — for a stat with no personal history at all — the
// position's league-wide per-game average, so a true rookie's projection
// still covers the same categories as anyone else's rather than silently
// running shorter. Each stat is tagged with where its number came from.
// Whatever's left after all three passes has no real basis for a number at
// all and is reported separately rather than guessed at — a fabricated
// figure would look exactly as authoritative as a real one, which is worse
// than an honest gap.
